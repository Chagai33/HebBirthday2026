    ...
    try {
        ...
        for (const doc of snapshot.docs) {
            ...
            if (!nextBirthday || nextBirthday < nowStr) {
                ...
                const upcomingDates = futureDates.filter((dateStr) => dateStr >= nowStr);
                        ...
                        if (hebrewYear && hebrewMonth && hebrewDay) {
                            ...
                            if (newFutureDates.length > 0) {
                                ...
                                batch.update(doc.ref, {
                                    ...
                                    future_hebrew_birthdays: newFutureDates.map((date) => `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`),
                                    updated_at: admin.firestore.FieldValue.serverTimestamp(),
                                });
                                updateCount++;
                            }
